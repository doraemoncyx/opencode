...
TUI. It's available by default.

Requests that arrive from the machine running the server are treated as already
authenticated, so opening the ui there needs no password. Requests from other
machines to the shared service authenticate with its password.

## Access

On the machine running the server, open the ui at its address. No credentials are
required.
...
From another machine, the server has to listen on a reachable interface and you
sign in with the service password.
...
$ opencode pair
  ...
  URLs      http://192.168.1.10:49374
  Username  opencode
  Password  ********
```

`opencode pair` lists the available URLs, the username and password, and a QR
code a client can scan.
